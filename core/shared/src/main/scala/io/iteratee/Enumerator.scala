...
final object Enumerator extends EnumeratorInstances {
  ...
  private[this] final def enumChunks[E, F[_]: Monad](
    chunks: Stream[Vector[E]]
  ): Enumerator[E, F] = new Enumerator[E, F] {
    def apply[A](s: Step[E, F, A]): Iteratee[E, F, A] = if (chunks.isEmpty) s.pointI else
      s.foldWith(
        new MapContStepFolder[E, F, A](s) {
          def onCont(k: Input[E] => Iteratee[E, F, A]): Iteratee[E, F, A] =
            k(Input.chunk(chunks.head)).feed(enumChunks[E, F](chunks.tail))
        }
      )
  }
  ...
  ): Enumerator[E, F] = enumChunks(xs.grouped(chunkSize).map(_.toVector).toStream)
