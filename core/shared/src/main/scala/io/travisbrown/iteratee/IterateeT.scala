...
sealed abstract class IterateeT[E, F[_], A] {
  ...
  def foldT[Z](
                ...
                , done: (=> A, => Input[E]) => F[Z]
                )(implicit F: FlatMap[F]): F[Z] =
    F.flatMap(value)((s: StepT[E, F, A]) => s(cont, done))

  /**
   * Run this iteratee
   */
  def run(implicit F: Monad[F]): F[A] = {
    F.flatMap((this &= enumEofT[E, F]).value)((s: StepT[E, F, A]) => s.fold(
   ...
   */
  def >>==[EE, AA](f: StepT[E, F, A] => IterateeT[EE, F, AA])(implicit F: FlatMap[F]): IterateeT[EE, F, AA] =
    iterateeT(F.flatMap(value)(s => f(s).value))

  def %=[O](e: EnumerateeT[O, E, F])(implicit m: Monad[F]): IterateeT[O, F, A] =
    (this >>== e[A]).joinI[E, A]

  def &=(e: EnumeratorT[E, F])(implicit F: FlatMap[F]): IterateeT[E, F, A] =
    this >>== e[A] 
  ...
  def up[G[_]](implicit G: Applicative[G], F: Comonad[F]): IterateeT[E, G, A] = {
    mapI(new NaturalTransformation[F, G] {
      def apply[A](a: F[A]) = G.pure(F.extract(a))
    })
  }
  ...
  def zip[B](other: IterateeT[E, F, B])(implicit F: Monad[F]): IterateeT[E, F, (A, B)] = {
    def step[Z](i: IterateeT[E, F, Z], in: Input[E]): IterateeT[E, F, (Option[(Z, Input[E])], IterateeT[E, F, Z])] =
      IterateeT.liftM(i.foldT[(Option[(Z, Input[E])], IterateeT[E, F, Z])](
        cont = k => F.pure((None, k(in)))
        , done = (a, x) => F.pure((Some((a, x)), done(a, x)))
      ))
...
trait IterateeTFunctions {
  def liftM[E, F[_] : Monad, A](ga: F[A]): IterateeT[E, F, A] =
    iterateeT(Monad[F].map[A, StepT[E, F, A]](ga)(a => sdone[E, F, A](a, emptyInput)))
   ...
   */
  def consume[E, F[_]: Monad, A[_]: MonoidK: Applicative]: IterateeT[E, F, A[E]] = {
    def step(e: Input[E]): IterateeT[E, F, A[E]] =
      e.fold(empty = cont(step)
        , el = e => cont(step).map(a => MonoidK[A].combine[E](Applicative[A].pure(e), a))
        , eof = done(MonoidK[A].empty[E], eofInput[E])
      )   
  ...
  def collectT[E, F[_], A[_]](implicit M: Monad[F], mae: Monoid[A[E]], pointed: Applicative[A]): IterateeT[E, F, A[E]] = {
    ...
    def step(e: Input[E]): IterateeT[E, F, A[E]] =
      e.fold(empty = cont(step)
        , el = e => cont(step).map(a => Applicative[A].pure(e) |+| a)
        , eof = done(Monoid[A[E]].empty, eofInput[E])
      )   
  ...
  def foldM[E, F[_], A](init: A)(f: (A, E) => F[A])(implicit m: Monad[F]): IterateeT[E, F, A] = {
    def step(acc: A): Input[E] => IterateeT[E, F, A] = s =>
      s(el = e => IterateeT.liftM(f(acc, e)) flatMap (a => cont(step(a))),
        empty = cont(step(acc)),
        eof = done(acc, eofInput[E]))
    cont(step(init))
  }
...
private trait IterateeTMonad[E, F[_]] extends Monad[IterateeT[E, F, ?]] {
  ...
  def pure[A](a: A): IterateeT[E, F, A] = StepT.sdone(a, emptyInput).pointI
  override def map[A, B](fa: IterateeT[E, F, A])(f: A => B): IterateeT[E, F, B] = fa map f
  def flatMap[A, B](fa: IterateeT[E, F, A])(f: A => IterateeT[E, F, B]): IterateeT[E, F, B] = fa flatMap f
}
