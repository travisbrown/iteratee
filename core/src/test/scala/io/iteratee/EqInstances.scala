...
trait EqInstances {
  ...
  implicit def eqIteratee[F[_]: Monad, A: Eq: Arbitrary, B: Eq: Arbitrary](implicit
    ...
    val e1 = Enumerator.enumEnd[F, A]
    val e2 = Enumerator.enumList[F, A](Arbitrary.arbitrary[List[A]].sample.get)
    val e3 = Enumerator.enumStream[F, A](Arbitrary.arbitrary[Stream[A]].sample.get)
    val e4 = Enumerator.enumVector[F, A](Arbitrary.arbitrary[Vector[A]].sample.get)
  ...
  implicit def eqEnumeratee[F[_]: Monad, A: Eq: Arbitrary, B: Eq: Arbitrary](implicit
    ...
    Eq.instance { (i, j) =>
      ...
      eq.eqv(e4.mapE(i).drain, e4.mapE(j).drain)
    }
  }
}
